...
PROJECT_ENV="${PROJECT_ENV:-prod}"

# AWS Resources
ECS_CLUSTER="${ECS_CLUSTER:-${PROJECT_NAME}-cluster}"
ECS_SERVICE="${ECS_SERVICE:-${PROJECT_NAME}-agent}"
LOG_GROUP_AGENT="${LOG_GROUP_AGENT:-/ecs/${PROJECT_NAME}-${PROJECT_ENV}-agent}"
DYNAMODB_TABLE="${DYNAMODB_TABLE:-${PROJECT_NAME}-${PROJECT_ENV}-sessions}"
SECRET_PATH="${SECRET_PATH:-/${PROJECT_NAME}/${PROJECT_ENV}/api-key}"

# Production URL
PROD_API_URL="${PROD_API_URL:-https://api.example.com}"

# AWS CLI wrapper path (relative to script dir)
AWS_CLI_WRAPPER="${AWS_CLI_WRAPPER:-aws-cli.sh}"

# SSO login hint
SSO_LOGIN_HINT="${SSO_LOGIN_HINT:-aws sso login}"
# ---- END CONFIGURATION ----

# Configuration
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
AWS_CLI="${SCRIPT_DIR}/${AWS_CLI_WRAPPER}"
...
print_summary() {
    ...
    if [[ ${CHECKS_FAILED} -gt 0 ]]; then
        ...
        echo "  - View logs:     ${AWS_CLI_WRAPPER} logs tail ${LOG_GROUP_AGENT} --follow"
        echo "  - Check secret:  ./scripts/secrets-manager-get.sh"
        echo "  - ECS service:   ${AWS_CLI_WRAPPER} ecs describe-services --cluster ${ECS_CLUSTER} --services ${ECS_SERVICE}"
        echo ""
        return 1
...
show_help() {
    ...
    echo "  PROD_API_URL    Production API URL (default: ${PROD_API_URL})"
    echo "  ECS_CLUSTER     ECS cluster name (default: ${ECS_CLUSTER})"
    echo "  ECS_SERVICE     ECS service name (default: ${ECS_SERVICE})"
    echo "  DYNAMODB_TABLE  DynamoDB table name (default: ${DYNAMODB_TABLE})"
    echo "  SECRET_PATH     Secrets Manager path (default: ${SECRET_PATH})"
    echo "  LOG_GROUP_AGENT CloudWatch log group (default: ${LOG_GROUP_AGENT})"
}
...
main() {
    ...
    echo -e "${BLUE}"
    echo "  +===============================================================+"
    echo "  |     ${PROJECT_NAME} - Remote Health Check                     |"
    echo "  +===============================================================+"
    echo -e "${NC}"
