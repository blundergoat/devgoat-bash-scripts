...
#
# USAGE:
#   ./scripts/secrets-manager-get.sh                 # Get the default secret
...
#   ./scripts/secrets-manager-get.sh /my-project/prod/api-key
...
SECRETS_PREFIX="${SECRETS_PREFIX:-/my-project}"
DEFAULT_SECRET_SUFFIX="${DEFAULT_SECRET_SUFFIX:-prod/api-key}"
# ---- END CONFIGURATION ----
...
View secrets from AWS Secrets Manager.
...
Arguments:
  secret-path   Full secret path (default: ${SECRET_PREFIX}/${DEFAULT_SECRET_SUFFIX})

Examples:
  $(basename "$0")                                    # Get the default secret
  $(basename "$0") ${SECRET_PREFIX}/${DEFAULT_SECRET_SUFFIX}  # Same, explicit path
...
# Default to the configured default secret
if [[ -z "${SECRET_NAME}" ]]; then
    SECRET_NAME="${SECRET_PREFIX}/${DEFAULT_SECRET_SUFFIX}"
fi
