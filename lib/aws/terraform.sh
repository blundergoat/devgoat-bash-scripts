...
# =============================================================================
#
# Simplifies common Terraform operations for your project.
#
# USAGE:
...
# Relative paths from project root to Terraform directories
TF_PROD_RELPATH="${TF_PROD_RELPATH:-infra/terraform/environments/prod}"
TF_BOOTSTRAP_RELPATH="${TF_BOOTSTRAP_RELPATH:-infra/terraform/bootstrap}"
TF_ROOT_RELPATH="${TF_ROOT_RELPATH:-infra/terraform}"
# ---- END CONFIGURATION ----

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
PROD_DIR="$PROJECT_ROOT/$TF_PROD_RELPATH"
BOOTSTRAP_DIR="$PROJECT_ROOT/$TF_BOOTSTRAP_RELPATH"
TF_ROOT="$PROJECT_ROOT/$TF_ROOT_RELPATH"
...
show_help() {
    echo -e "${BOLD}${BLUE}Terraform Helper Script - ${PROJECT_NAME}${NC}"
    echo ""
...
run_terraform() {
    ...
    echo -e "${CYAN}Profile: $AWS_PROFILE | Region: $AWS_REGION${NC}"
    echo ""
