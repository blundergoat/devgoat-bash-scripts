...
# =============================================================================
# Usage: ./install-ollama.sh [--port PORT] [--model MODEL] [--force]
#
...
#   5. Updates .env with OLLAMA_HOST (if .env exists in project root)
#
# Options:
...
#   --project-root DIR  Project root containing .env (default: parent of script dir)
#
...
for arg in "$@"; do
    case "$arg" in
        ...
        --project-root) shift_next=project_root ;;
        ...
        *)
            if [[ "${shift_next}" == "port" ]]; then
                ...
                OLLAMA_MODEL="$arg"
                shift_next=""
            elif [[ "${shift_next}" == "project_root" ]]; then
                PROJECT_ROOT="$arg"
                shift_next=""
            else
                echo "Unknown option: $arg"
                echo "Usage: $0 [--port PORT] [--model MODEL] [--project-root DIR] [--force]"
                exit 1
            fi
            ;;
    esac
...
if [[ -z "$OLLAMA_MODEL" && -f "$PROJECT_ROOT/.env" ]]; then
    OLLAMA_MODEL="$(grep -E '^OLLAMA_MODEL=' "$PROJECT_ROOT/.env" 2>/dev/null | head -1 | cut -d= -f2-)"
fi
...
if [[ -f "$PROJECT_ROOT/.env" ]]; then
    ...
    if grep -q '^OLLAMA_HOST=' "$PROJECT_ROOT/.env" 2>/dev/null; then
        CURRENT_HOST=$(grep -E '^OLLAMA_HOST=' "$PROJECT_ROOT/.env" | head -1 | cut -d= -f2-)
        if [[ "$CURRENT_HOST" != "$EXPECTED_HOST" ]]; then
            sed -i "s|^OLLAMA_HOST=.*|OLLAMA_HOST=${EXPECTED_HOST}|" "$PROJECT_ROOT/.env"
            step "OLLAMA_HOST"
        ...
        if grep -q '^OLLAMA_MODEL=' "$PROJECT_ROOT/.env"; then
            sed -i "/^OLLAMA_MODEL=/a OLLAMA_HOST=${EXPECTED_HOST}" "$PROJECT_ROOT/.env"
        else
            echo "OLLAMA_HOST=${EXPECTED_HOST}" >> "$PROJECT_ROOT/.env"
        fi
        step "OLLAMA_HOST"
    ...
    if grep -q '^OLLAMA_MODEL=' "$PROJECT_ROOT/.env" 2>/dev/null; then
        CURRENT_MODEL=$(grep -E '^OLLAMA_MODEL=' "$PROJECT_ROOT/.env" | head -1 | cut -d= -f2-)
        if [[ "$CURRENT_MODEL" != "$OLLAMA_MODEL" ]]; then
            sed -i "s|^OLLAMA_MODEL=.*|OLLAMA_MODEL=${OLLAMA_MODEL}|" "$PROJECT_ROOT/.env"
            step "OLLAMA_MODEL"
...
if [[ $ERRORS -eq 0 ]]; then
    ...
    if [[ "$IS_WSL" == true ]]; then
        ...
        echo -e "  ${DIM}It will stop when you close this terminal.${RESET}"
        echo ""
    fi
else
