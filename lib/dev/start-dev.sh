#!/usr/bin/env bash
# =============================================================================
# Start Dev - Launches backend + frontend services for local development
# =============================================================================
...
#     2. Python FastAPI agent on configured port
#     3. Mercure hub (if MERCURE_URL is set in .env)
#     4. PHP Symfony dev server on configured port
#
#   bedrock:
#     1. Python FastAPI agent on configured port
#     2. Mercure hub (if MERCURE_URL is set in .env)
#     3. PHP Symfony dev server on configured port
...
PYTHON_SUBDIR="${PYTHON_SUBDIR:-agent}"
VENV_SUBDIR="${VENV_SUBDIR:-.venv}"
AGENT_APP_MODULE="${AGENT_APP_MODULE:-api.server:app}"
PHP_PUBLIC_DIR="${PHP_PUBLIC_DIR:-public}"
DEPENDENCY_SCRIPT="${DEPENDENCY_SCRIPT:-scripts/dependencies-install.sh}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-docker-compose.yml}"
MERCURE_CONTAINER_NAME="${MERCURE_CONTAINER_NAME:-${PROJECT_NAME}-mercure-dev}"
DEFAULT_OLLAMA_MODEL="${DEFAULT_OLLAMA_MODEL:-qwen2.5:14b}"
HEADER_TEXT="${HEADER_TEXT:-${PROJECT_NAME} - Dev Server}"
# ---- END CONFIGURATION ----

REPO_ROOT="$(git -C "$(dirname "$0")" rev-parse --show-toplevel 2>/dev/null || cd "$(dirname "$0")/.." && pwd)"
PYTHON_AGENT_DIR="$REPO_ROOT/$PYTHON_SUBDIR"
VENV_DIR="$PYTHON_AGENT_DIR/$VENV_SUBDIR"
...
cleanup() {
    ...
    if [[ "$MERCURE_DOCKER" == "true" ]]; then
        docker stop "$MERCURE_CONTAINER_NAME" >/dev/null 2>&1 || true
        docker rm "$MERCURE_CONTAINER_NAME" >/dev/null 2>&1 || true
    fi
...
if command -v docker &>/dev/null; then
    running=$(docker ps --filter "name=${PROJECT_NAME}" --format "{{.Names}}" 2>/dev/null || true)
    if [[ -n "$running" ]]; then
        ...
        if docker compose -f "$REPO_ROOT/$DOCKER_COMPOSE_FILE" down >/dev/null 2>&1; then
...
if [[ ! -f "$VENV_DIR/bin/uvicorn" || ! -f "$REPO_ROOT/vendor/autoload.php" ]]; then
    ...
    echo ""
    echo -e "  ${ARROW} Running ${BOLD}./${DEPENDENCY_SCRIPT}${RESET}..."
    echo ""
    if "$REPO_ROOT/$DEPENDENCY_SCRIPT"; then
        echo ""
...
env_default OLLAMA_MODEL    "$DEFAULT_OLLAMA_MODEL"
...
"$VENV_DIR/bin/uvicorn" "$AGENT_APP_MODULE" \
    --host 0.0.0.0 \
    --port "$AGENT_PORT" \
    --log-level warning \
...
if [[ -n "$MERCURE_URL" && -n "$MERCURE_JWT_SECRET" ]]; then
    if ! command -v docker &>/dev/null; then
        ...
        if docker ps -a --filter "name=^${MERCURE_CONTAINER_NAME}$" --format "{{.Names}}" 2>/dev/null | grep -q "^${MERCURE_CONTAINER_NAME}$"; then
            docker rm -f "$MERCURE_CONTAINER_NAME" >/dev/null 2>&1 || true
        fi

        echo -e "  ${ARROW} Mercure                ${DIM}http://localhost:${MERCURE_PORT}${RESET}"
        if docker run -d --name "$MERCURE_CONTAINER_NAME" \
...
for i in $(seq 1 15); do
    ...
    if [[ $i -eq 15 ]]; then
        ...
        if [[ "$MERCURE_READY" != "true" ]]; then
            ...
            echo -e "     ${DIM}Check: docker logs ${MERCURE_CONTAINER_NAME}${RESET}"
            echo -e "     ${DIM}Falling back to sync mode${RESET}"
            MERCURE_DOCKER=false
        fi
    fi
...
php -S "0.0.0.0:${APP_PORT}" -t "$REPO_ROOT/$PHP_PUBLIC_DIR" \
    > >(while IFS= read -r line; do
