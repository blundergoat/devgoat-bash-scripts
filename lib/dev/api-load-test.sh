...
# =============================================================================
#
# Performance testing for an agent API using 'hey', 'ab', or 'wrk'.
#
# USAGE:
...
#   ./scripts/api-load-test.sh --url https://api.example.com
#
# =============================================================================
...
DEFAULT_REQUESTS="${DEFAULT_REQUESTS:-200}"
DEFAULT_CONCURRENCY="${DEFAULT_CONCURRENCY:-10}"
DEFAULT_ENDPOINT="${DEFAULT_ENDPOINT:-/health}"
RATE_LIMIT_NOTE="${RATE_LIMIT_NOTE:-WAF rate limiting may be configured. High concurrency tests may trigger rate limiting (429 responses).}"
# ---- END CONFIGURATION ----
...
show_help() {
    echo ""
    echo -e "${BLUE}API Load Testing Script - ${PROJECT_NAME}${NC}"
    echo ""
    echo "Usage:"
    echo "  $0 [options]"
    echo ""
    echo "Options:"
    echo "  -e, --endpoint PATH    API endpoint to test (default: ${DEFAULT_ENDPOINT})"
    echo "  -n, --requests NUM     Total number of requests (default: ${DEFAULT_REQUESTS})"
    echo "  -c, --concurrency NUM  Concurrent requests (default: ${DEFAULT_CONCURRENCY})"
    ...
    echo "  /invoke                Synchronous agent invocation (slow, calls LLM)"
    echo "  /stream                Streaming agent invocation (SSE)"
    echo ""
    echo "Examples:"
    echo "  $0                                       # Default: ${DEFAULT_REQUESTS} req to ${DEFAULT_ENDPOINT}"
    ...
    echo "  $0 --url https://api.example.com          # Test production"
...
check_api() {
    ...
    if curl -sf "${health_url}" > /dev/null 2>&1; then
        ...
        echo "  export API_URL=https://api.example.com"
        exit 1
    fi
}
...
run_suite() {
    echo ""
    echo -e "${BLUE}=============================================="
    echo "  ${PROJECT_NAME} - Endpoint Test Suite"
    echo "==============================================${NC}"
    echo ""
    ...
    echo -e "because they call the LLM and are expensive. Test manually:${NC}"
...
main() {
    echo ""
    echo -e "${BLUE}=============================================="
    echo "  ${PROJECT_NAME} - API Load Testing"
    echo "==============================================${NC}"
    echo ""
