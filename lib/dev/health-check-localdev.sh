...
#   - Ollama API (configurable)
#   - Python FastAPI agent (configurable port)
#   - PHP Symfony app (configurable port)
#   - Mercure hub (configurable port) - optional
#
...
HEADER_TEXT="${HEADER_TEXT:-${PROJECT_NAME} - Health Check}"
DEFAULT_AGENT_PORT="${DEFAULT_AGENT_PORT:-8081}"
DEFAULT_APP_PORT="${DEFAULT_APP_PORT:-8082}"
DEFAULT_OLLAMA_HOST="${DEFAULT_OLLAMA_HOST:-http://localhost:11434}"
DEFAULT_MERCURE_PORT="${DEFAULT_MERCURE_PORT:-3701}"
DEFAULT_OLLAMA_MODEL="${DEFAULT_OLLAMA_MODEL:-qwen2.5:14b}"
DEFAULT_MODEL_PROVIDER="${DEFAULT_MODEL_PROVIDER:-ollama}"
# Agent endpoints to health-check
AGENT_HEALTH_PATH="${AGENT_HEALTH_PATH:-/health}"
AGENT_DOCS_PATH="${AGENT_DOCS_PATH:-/docs}"
AGENT_INVOKE_PATH="${AGENT_INVOKE_PATH:-/invoke}"
AGENT_STREAM_PATH="${AGENT_STREAM_PATH:-/stream}"
# PHP app endpoints
APP_ROOT_PATH="${APP_ROOT_PATH:-/}"
APP_POST_PATH="${APP_POST_PATH:-/chat}"
...
MERCURE_PORT="${MERCURE_PORT:-$DEFAULT_MERCURE_PORT}"
...
probe "http://localhost:${AGENT_PORT}${AGENT_HEALTH_PATH}"
if [[ "$PROBE_STATUS" == "200" ]]; then
...
step "OpenAPI docs"
probe "http://localhost:${AGENT_PORT}${AGENT_DOCS_PATH}"
if [[ "$PROBE_STATUS" == "200" ]]; then
    pass "${PROBE_TIME_MS}ms"
else
...
step "POST ${AGENT_INVOKE_PATH} accepts requests"
...
invoke_result=$(curl -s -o /dev/null -w "%{http_code}" \
    ...
    -X POST "http://localhost:${AGENT_PORT}${AGENT_INVOKE_PATH}" 2>/dev/null) || true
...
step "POST ${AGENT_STREAM_PATH} accepts requests"
stream_result=$(curl -s -o /dev/null -w "%{http_code}" \
    --connect-timeout 3 --max-time 5 \
    -X POST "http://localhost:${AGENT_PORT}${AGENT_STREAM_PATH}" 2>/dev/null) || true
...
step "GET ${APP_ROOT_PATH} (app UI)"
probe "http://localhost:${APP_PORT}${APP_ROOT_PATH}"
if [[ "$PROBE_STATUS" == "200" ]]; then
    pass "${PROBE_TIME_MS}ms"
else
    fail "not reachable - is start-dev.sh running?"
...
if [[ "$PROBE_STATUS" == "200" ]]; then
    step "Renders HTML"
    if echo "$PROBE_BODY" | grep -qiE "${APP_HTML_PATTERN}"; then
        pass
    else
...
step "POST ${APP_POST_PATH} rejects empty"
chat_result=$(curl -sf -w "\n%{http_code}" \
    --connect-timeout 3 --max-time 5 \
    -X POST "http://localhost:${APP_PORT}${APP_POST_PATH}" \
