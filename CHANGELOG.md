...
- **`dashboard/start.sh` project name** — default changed from `my-project` to `DevGoat DevEx Dashboard`.
- **`dashboard/start.sh` SCRIPTS_DIR** — default changed from `scripts` to `.` (project root).
- **`dashboard/start.sh` logging** — filters PHP built-in server TCP noise, only shows `[dashboard]` tagged events.
...
- **`dashboard/start.sh` config.php** — changed from hard-fail to auto-copy from `config.example.php` with warning.
...
- **`dashboard/start.sh`** — template launcher for the PHP script-runner web UI (localhost-only).
